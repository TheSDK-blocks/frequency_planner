...
class frequency_planner(rtl,eldo,thesdk):
    @property
    def _classfile(self):
    ...
    def run(self,*arg):
        ...
        if self.model=='py':
          ...
          if self.model=='vhdl':
              ...
              # Example of defining supplies (not used here because the example frequency_planner has no supplies)
...
if __name__=="__main__":
    ...
    from  frequency_planner.controller import controller as frequency_planner_controller
    import pdb
    length=1024
    rs=100e6
    indata=np.random.randint(2,size=length).reshape(-1,1);
    #indata=np.random.randint(2,size=length)
    controller=frequency_planner_controller()
    controller.Rs=rs
    #controller.reset()
